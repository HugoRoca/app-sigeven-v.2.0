	...
	case 'listar':
		$rspta=$venta->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			$data[]=array(
 				"0"=>(($reg->estado=='ACTIVO')?'<button class="btn btn-warning" onclick="mostrar('.$reg->idventa.')"><i class="fa fa-eye"></i></button>'.
 				...
 				"6"=>$reg->total_venta,
 				"7"=>($reg->estado=='ACTIVO')?'<span class="label bg-green">ACTIVO</span>':
